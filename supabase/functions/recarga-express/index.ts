    ...
    switch (action) {
      ...
      case "recharge": {
        ...
        if (dmResult.applied) {
          ...
          // 2. Fallback: if user has reseller_id, use reseller's pricing (not own rules)
          if (!clientPricingApplied && resellerId) {
            const { data: resellerRule } = await adminClient
              .from("reseller_pricing_rules")
              .select("*")
              .eq("user_id", resellerId)
        ...
        if (!rechargeResult?.success) {
          ...
          if (lowerErr.includes("limite de crédito") || lowerErr.includes("credit limit") || lowerErr.includes("saldo insuficiente") || lowerErr.includes("insufficient balance")) {
            ...
            try {
              ...
              let MASTER_TELEGRAM_ID = 1901426549;
              try {
                const cfgRes = await fetch(`${baseUrl}/rest/v1/system_config?key=eq.supportAdminTelegramId&select=value`, {
                  headers: { ...authH, apikey: Deno.env.get("SUPABASE_ANON_KEY") || svcKey },
                });
                const cfgRows = await cfgRes.json();
                if (cfgRows?.[0]?.value) MASTER_TELEGRAM_ID = Number(cfgRows[0].value) || 1901426549;
              } catch { /* fallback */ }
