...
export function PollManager() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <AnimatePresence>
        {showCreate && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[70] p-4" onClick={() => setShowCreate(false)}>
            <motion.div
      ...
      <AnimatePresence>
        {selectedPoll && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[70] p-4" onClick={() => setSelectedPoll(null)}>
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        {poll.active ? (
                          <span className="px-2.5 py-1 rounded-full bg-success/15 text-success text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
                            <span className="w-1.5 h-1.5 rounded-full bg-success animate-pulse" />
                            Ativa
                          </span>
                        ) : (
                          <span className="px-2.5 py-1 rounded-full bg-muted/60 text-muted-foreground text-[10px] font-bold uppercase tracking-wider">
                            Inativa
                          </span>
                        )}
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <button onClick={() => { if (confirm("Excluir esta enquete?")) deletePoll(poll.id); }} title="Excluir"
                    ...
                    return (
                      <div key={i}>
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className={`font-medium truncate mr-2 ${isWinner ? "text-primary" : "text-foreground"}`}>
                            {opt.label}
                          </span>
                          <span className="text-muted-foreground tabular-nums shrink-0">
                            {opt.votes || 0} <span className="opacity-60">({pct}%)</span>
                          </span>
                        </div>
