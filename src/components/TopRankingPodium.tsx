  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {showPodium && (
        <div className="flex items-end justify-center gap-3 sm:gap-6 lg:gap-10 pt-4 pb-2">
          {podiumOrder.map((user, displayIndex) => {
            ...
            return (
              <motion.div
                key={user.user_id}
                className={`flex flex-col items-center gap-2 cursor-pointer ${isCenter ? "mb-4 w-[100px] lg:w-[160px]" : "mb-0 w-[80px] lg:w-[130px]"}`}
                style={{ flexShrink: 0 }}
