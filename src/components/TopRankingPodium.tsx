      ...
      {restUsers.length > 0 && (
        ...
        >
          {restUsers.map((user, index) => {
            ...
            const pct = (user.total_recargas / maxRecargas) * 100;
            const isTopRow = globalIndex < 3;

            return (
              <motion.div
                key={user.user_id}
                variants={{ hidden: { opacity: 0, x: -16 }, visible: { opacity: 1, x: 0 } }}
                className={`flex items-center gap-2.5 rounded-lg px-2.5 py-2 transition-all hover:bg-muted/40 ${
                  isTopRow ? rowStyles[globalIndex] : ""
                } ${isMe && !isTopRow ? "bg-primary/5 border-l-2 border-l-primary" : ""}`}
              >
                {/* Position */}
                <div className="w-7 h-7 rounded-md flex items-center justify-center font-bold text-xs shrink-0 bg-muted/60 text-muted-foreground">
                  <span className="text-[10px]">#{globalIndex + 1}</span>
                </div>
                ...
                <div className="relative w-8 h-8 rounded-full overflow-hidden shrink-0">
                  {user.avatar_url ? (
                    ...
                    <div className="w-full h-full bg-muted flex items-center justify-center text-foreground text-xs font-bold">
                      {(user.nome?.[0] || "?").toUpperCase()}
                    </div>
                  )}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-0.5">
                    <div className="flex items-center gap-1 min-w-0">
                      <p className="text-[13px] font-medium text-foreground truncate">{user.nome}</p>
                      {user.verification_badge && (
                        <VerificationBadge badge={user.verification_badge as BadgeType} size="sm" />
                      )}
                      {isMe && (
                        <span className="text-[9px] bg-primary/20 text-primary px-1.5 py-0.5 rounded-full font-semibold shrink-0">Você</span>
                      )}
                    </div>
                    <span className="font-mono font-semibold text-[12px] tabular-nums text-foreground shrink-0">
                      {user.total_recargas}
                    </span>
                  </div>
