...
export function SplashScreen() {
  ...
  /* ── Single canvas: background particles + disintegration burst ── */
  useEffect(() => {
    ...
    const spawnBurst = () => {
      ...
      const cx = w / 2;
      const cy = h / 2;
      const logoSize = 56; // half of ~112px logo
      // Spawn ~200 particles from logo area
      for (let i = 0; i < 200; i++) {
        const ox = cx + (Math.random() - 0.5) * logoSize * 2;
        const oy = cy + (Math.random() - 0.5) * logoSize * 2;
        const angle = Math.random() * Math.PI * 2;
        const speed = 1.5 + Math.random() * 4;
        const hue = 140 + Math.random() * 30; // green range
    ...
    const loop = () => {
      ...
      // Ambient particles (only if not disintegrating yet or still some alive)
      if (particles.length < 60 && !burstSpawned) {
        spawnAmbient();
  ...
  /* ── Trigger burst when disintegrating ── */
  useEffect(() => {
    ...
    for (let i = 0; i < 250; i++) {
      const ox = cx + (Math.random() - 0.5) * logoSize * 2;
      const oy = cy - 20 + (Math.random() - 0.5) * logoSize * 2;
      const angle = Math.atan2(oy - cy, ox - cx) + (Math.random() - 0.5) * 0.8;
      const speed = 2 + Math.random() * 5;
      // We need to add to the existing particle array — but it's in a different useEffect closure
      // Instead, let's dispatch a custom event
      window.dispatchEvent(new CustomEvent("splash-burst", {
        detail: { x: ox, y: oy, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed, r: 1 + Math.random() * 3, alpha: 0.9, decay: 0.005 + Math.random() * 0.01 }
      }));
    }
  }, [disintegrating]);
    ...
    >
      ...
      <div className="relative z-10" style={{ animation: "splash-logo-enter 0.8s cubic-bezier(0.16,1,0.3,1) both" }}>
        ...
        <div className="w-24 h-24 md:w-28 md:h-28 rounded-2xl overflow-hidden border border-border/30"
          style={{
            ...
            transform: disintegrating ? "scale(1.1)" : "scale(1)",
          }}
        >
      ...
      <h1 className="relative z-10 mt-5 text-lg font-bold tracking-wider"
        style={{
          ...
          animationDuration: "0.6s, 3s", animationDelay: "0.3s, 1s",
          animationTimingFunction: "cubic-bezier(0.16,1,0.3,1), ease-in-out",
          animationFillMode: "both, none", animationIterationCount: "1, infinite",
        }}
      >
        {siteName}
      </h1>
